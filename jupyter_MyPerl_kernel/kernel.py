...
class MyPerlKernel(MyKernel):
    implementation = 'jupyter-MyPerl-kernel'
    implementation_version = '1.0'
    language = 'Perl'
    language_version = ''
    language_info = {'name': 'Perl',
                     ...
                     'file_extension': '.pl'}
    runfiletype='script'
    banner = "MyPerl kernel.\n" \
             "Uses perl, creates source code files and executables in temporary folder.\n"
    kernelinfo="[MyPerl]"
    main_head = "\n" \
            "\n" \
...
##//%include:src/comm_attribute.py
    def __init__(self, *args, **kwargs):
        super(MyPerlKernel, self).__init__(*args, **kwargs)
        self.runfiletype='script'
        self.kernelinfo="[MyPerlKernel{0}]".format(time.strftime("%H%M%S", time.localtime()))
...
##do_runcode
    def do_runcode(self,return_code,file_name,magics,code, silent, store_history=True,
        ...
        retstr=''
        ##代码运行前
        switches=self.mymagics.get_magicsSvalue(magics,'switches')
        perlcmd=['perl']
        if len(switches)>0:
            perlcmd+=switches+['--']
        else:
            perlcmd+=['--']
        p = self.mymagics.create_jupyter_subprocess(perlcmd+[file_name]+ magics['_st']['args'],cwd=None,shell=False,env=self.mymagics.addkey2dict(magics,'env'),magics=magics)
...
##do_create_codefile
    def do_create_codefile(self,magics,code, silent, store_history=True,
        ...
        retinfo=self.mymagics.get_retinfo()
        retstr=''
        source_file=self.mymagics.create_codetemp_file(magics,code,suffix='.perl')
